...
TxtPth = "/media/g/P04/Psychophysics/"
...
# Save the dataframe     
dfObj00.to_excel(TxtPth + 'All_subjects_lum_pt37.xls')
dfObj00.to_csv(TxtPth + 'All_subjects_lum_pt37.csv')
...
mean_ary = np.mean(np.asarray(P_MED),axis=0) - (-0.37)
...
plt.show()   
# Save 
plt.savefig('All_sub_allCond_CPL_mdm_pt37.png',dpi=300)
plt.savefig('All_sub_allCond_CPL_mdm_pt37.svg',dpi=300)
...
dfObj01.to_excel(TxtPth + 'All_subjects_lum_pt37_anova.xls')
dfObj01.to_csv(TxtPth + 'All_subjects_lum_pt37_anova.csv') 

# Statistics
res = pg.rm_anova(data=dfObj01, dv='M_Plum_vs_Base', subject='Subject', within=['Luminance', 'Condition_grp'],detailed=True)
res.to_csv(TxtPth + 'Group_two_way_rm_anova_pt37_pg.csv')
